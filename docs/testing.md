...
swift run passsync simulate \
  --input Examples/simulation-state.json \
  --direction bidirectional \
  --output /tmp/passsync-state-decisions.json \
  --record-state \
  --state-path /tmp/passsync-test-state.sqlite
